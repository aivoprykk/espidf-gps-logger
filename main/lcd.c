...
static esp_err_t speed_info_bar_update() {  // info bar when config->screen.speed_large_font is 1 or 0
    ...
    // More then 1852m : NM actual speed and NM Best speed
        ...
        { // alfa speed stats
            ...
            if (font_size == 0) {
                var[0] = scr_fld[0][cfg_fld_spd_alpha][0];
                var[1] = scr_fld[0][cfg_fld_spd_alpha][1];  // nieuwe alfa laatste gijp or MISSED !!!!
            } else {
                // best Alfa from session on 500 m !!
                var[0] = scr_fld[1][cfg_fld_spd_alpha][0];
                var[1] = scr_fld[1][cfg_fld_spd_alpha][1];
            }
        }
    }
        ...
        if (font_size == 0){
            ...
            *val[1] = 0;
        if (font_size == 0) {
            var[0] = scr_fld[0][cfg_fld_spd_1852_m][0];  // Actuele nautical mile
            var[1] = scr_fld[0][cfg_fld_spd_1852_m][1];
        } else {
            var[0] = scr_fld[1][cfg_fld_spd_1852_m][0];  // Actuele nautical mile
            var[1] = scr_fld[1][cfg_fld_spd_1852_m][1];
        }
    }
    else if (field == cfg_fld_spd_dist_time) { // total distance
        s[0] = avail_fields[fld_distance].value.num(); // total dist km
        var[0] = scr_fld[0][cfg_fld_spd_dist_time][0];
        var[1] = scr_fld[0][cfg_fld_spd_dist_time][1];
        if (NUM_LT_3_DIG(s[0])) 
            f2_to_char(s[0], val[0]);
        else
            f1_to_char(s[0], val[0]);
        avail_fields[fld_total_time_hms].value.timestr(val[1]); // total time
    }
        ...
        var[0] = scr_fld[0][cfg_fld_spd_500_m][0];
        var[1] = scr_fld[0][cfg_fld_spd_500_m][1];
        if (NUM_LT_3_DIG(s[0])) 
            f2_to_char(s[0], val[0]);
        else
            f1_to_char(s[0], val[0]);
        ...
        if (NUM_LT_3_DIG(s[1]))
            ...
            f1_to_char(s[1], val[1]);
        var[0] = scr_fld[0][cfg_fld_spd_max_2s_10s][0];
        var[1] = scr_fld[0][cfg_fld_spd_max_2s_10s][1];
    }
        ...
        if (NUM_LT_3_DIG(s[1]))
            ...
            f1_to_char(s[1], val[1]);
        var[0] = scr_fld[0][cfg_fld_spd_half_hour][0];
        var[1] = scr_fld[0][cfg_fld_spd_half_hour][1];
    }
        ...
        if (NUM_LT_3_DIG(s[1]))
            ...
            f1_to_char(s[1], val[1]);
        var[0] = scr_fld[0][cfg_fld_spd_1_hour][0];
        var[1] = scr_fld[0][cfg_fld_spd_1_hour][1];
    }
