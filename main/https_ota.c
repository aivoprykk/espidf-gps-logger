    ...
    if (event_base == ESP_HTTPS_OTA_EVENT) {
        switch (event_id) {
            ...
            case ESP_HTTPS_OTA_WRITE_FLASH:
                ESP_LOGD(TAG, "Writing to flash: %d written",
                         *(int *)event_data);
                break;
...
static esp_err_t validate_image_header(esp_app_desc_t *new_app_info) {
    ...
    if (memcmp(new_app_info->version, running_app_info.version,
        ...
        ESP_LOGW(TAG, "Current running version is the same as a new. We will "
    ...
    if (new_app_info->secure_version < hw_sec_version) {
        ESP_LOGW(TAG,
                 "New firmware security version is less than eFuse programmed, "
                 "%" PRIu32 " < %" PRIu32,
